...
void Calculation(void)
{
	...
	//========================================================
i=0;
	for(j = 0; j < nbun_pole; ++j){
		//--始点 終点 セグメント長計算
		rrx = -0.25*r_pole + (j+0)*(r_pole/n_pole);		//始点位置計算x
		rry = 0;												//始点位置計算y
		rrz = 0.25*RAMDA0;												//始点位置計算z
		drx = -0.25*r_pole + (j+1)*(r_pole/n_pole);		//終点位置計算x
		dry = 0;												//終点位置計算y
		drz = 0.25*RAMDA0;		//終点位置計算z
	...
	FEDP[0] = SEGN[0]/2 - 1;
	FEDP[1] = SEGN[1]/2 - 1;
	FEDV[0] = -1.0*Complex(1.0 , 0.0);
	FEDV[1] = 1.0*Complex(1.0 , 0.0);
