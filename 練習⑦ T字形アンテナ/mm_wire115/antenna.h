...
void Calculation(void)
{
	...
	//--各部分の分割数
	int nbun_pole1,nbun_pole2,nbun_pole3;						//ダイポールの分割数
	
	//--各部のセグメント数
	int n_pole;							//ダイポールのセグメント数

	//--設計値関係
	double r_pole1,r_pole2,r_pole3,r_pole; //ダイポールの長さ
	...
	nbun_pole1 = 20;//a
	nbun_pole2 = 20;//b
	nbun_pole3 = 8;//c//ダイポールの分割数

	//--全セグメント数設定
	n_pole = nbun_pole1+nbun_pole2+nbun_pole3+nbun_pole3+nbun_pole2+nbun_pole1+3;					
	NSEG = n_pole;						//全セグメント数
...
for(j = 0; j < nbun_pole1+nbun_pole2; ++j){
		//--始点 終点 セグメント長計算
		rrx = (j-nbun_pole2)*(r_pole1/nbun_pole2);//始点位置計算x
		rry = 0;//始点位置計算y
		rrz = 0.1*RAMDA0;	//始点位置計算z																								
		drx = (j-nbun_pole2+1)*(r_pole1/nbun_pole2);												//終点位置計算x
		dry = 0;											//終点位置計算y
		drz = 0.1*RAMDA0;		//終点位置計算z
...
for(j = 0; j < 1; ++j){
		//--始点 終点 セグメント長計算
		rrx = (j-1)*(r_pole1/nbun_pole2);												//始点位置計算x
		rry = 0;												//始点位置計算y
		rrz = 0.1*RAMDA0;	//始点位置計算z
		drx = (j+0)*(r_pole1/nbun_pole2);												//終点位置計算x
		dry = 0;											//終点位置計算y
		drz = 0.1*RAMDA0;		//終点位置計算z
		//--セグメント長 単位方向ベクトルの各成分の計算
		dcl = sqrt( pow( (rrx-drx),2 )
...
for(j = 0; j < nbun_pole3*2; ++j){
		//--始点 終点 セグメント長計算
		rrx = 0;												//始点位置計算x
		rry = 0;												//始点位置計算y
		rrz = (nbun_pole3-j)*(r_pole3/nbun_pole3);		//始点位置計算z
		drx = 0;												//終点位置計算x
		dry = 0;											//終点位置計算y
		drz = (nbun_pole3-j-1)*(r_pole3/nbun_pole3);		//終点位置計算z
		//--セグメント長 単位方向ベクトルの各成分の計算
		dcl = sqrt( pow( (rrx-drx),2 )
				  + pow( (rry-dry),2 )
				  + pow( (rrz-drz),2 ) );						//セグメント長計算
...
for(j = 0; j < nbun_pole1; ++j){
		//--始点 終点 セグメント長計算
		rrx = (j-nbun_pole2)*(r_pole1/nbun_pole1);												//始点位置計算x
		rry = 0;												//始点位置計算y
		rrz = temp;	//始点位置計算z
		drx = (j-nbun_pole2+1)*(r_pole1/nbun_pole1);												//終点位置計算x
		dry = 0;											//終点位置計算y
		drz = temp;		//終点位置計算z
		//--セグメント長 単位方向ベクトルの各成分の計算
		dcl = sqrt( pow( (rrx-drx),2 )
...
for(j = 0; j < 1; ++j){
		...
		drx = 0;												//終点位置計算x
		dry = 0;											//終点位置計算y
		drz = (nbun_pole3-nbun_pole3*2+1)*(r_pole3/nbun_pole3);		//終点位置計算z
		//--セグメント長 単位方向ベクトルの各成分の計算
		dcl = sqrt( pow( (rrx-drx),2 )
				  + pow( (rry-dry),2 )
				  + pow( (rrz-drz),2 ) );						//セグメント長計算
...
for(j = 0; j < 1; ++j){
		...
		rrx = 0;												//始点位置計算x
		rry = 0;												//始点位置計算y
		rrz = (nbun_pole3-nbun_pole3*2+1)*(r_pole3/nbun_pole3);		//始点位置計算z
		drx = 0;												//終点位置計算x
		dry = 0;											//終点位置計算y
		drz = temp;		//終点位置計算ztemp
		//--セグメント長 単位方向ベクトルの各成分の計算
		dcl = sqrt( pow( (rrx-drx),2 )
				  + pow( (rry-dry),2 )
				  + pow( (rrz-drz),2 ) );						//セグメント長計算
...
for(j = 0; j < nbun_pole2; ++j){
		//--始点 終点 セグメント長計算
		rrx = (j+0)*(r_pole2/nbun_pole2);												//始点位置計算x
		rry = 0;												//始点位置計算y
		rrz = temp;	//始点位置計算z
		drx = (j+1)*(r_pole2/nbun_pole2);												//終点位置計算x
		dry = 0;											//終点位置計算y
		drz = temp;		//終点位置計算z
		//--セグメント長 単位方向ベクトルの各成分の計算
		dcl = sqrt( pow( (rrx-drx),2 )
	...
	//========================================================
//*	
	SEGN[0] = nbun_pole1+nbun_pole2;
    SEGN[1] = nbun_pole3*2+1;
	SEGN[2] = nbun_pole1+1;
	SEGN[3] = nbun_pole2+1;
//*/
	//========================================================
